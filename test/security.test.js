...
test('Stripe Connect uses signed Standard OAuth instead of platform-created accounts', () => {
  ...
  assert.match(connect, /return_origin:\s*returnOrigin/);
  ...
  assert.match(callback, /state\.return_origin/);
});
...
test('subscription checkout has no sandbox price fallbacks', () => {
  const subscribe = readFileSync('api/stripe-subscribe.js', 'utf8');
  const subscriptions = readFileSync('lib/stripe-subscriptions.js', 'utf8');
  assert.doesNotMatch(subscribe, /price_1TKN/);
  assert.doesNotMatch(subscriptions, /price_1TKN/);
  assert.match(subscribe, /Stripe subscription prices are not configured/);
});
